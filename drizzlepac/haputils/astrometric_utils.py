...
def create_astrometric_catalog(inputs, catalog="GAIAedr3", output="ref_cat.ecsv",
    ...
    existing_wcs : `stwcs.wcsutil.HSTWCS <https://stwcs.readthedocs.io/en/latest/hstwcs.html>`_
...
def compute_radius(wcs):
    ...
    Parameters
    -----------
    wcs : `stwcs.wcsutil.HSTWCS <https://stwcs.readthedocs.io/en/latest/hstwcs.html>`_ object
...
def compute_2d_background(imgarr, box_size, win_size,
    ...
    <https://photutils.readthedocs.io/en/stable/api/photutils.background.Background2D.html#photutils.background.Background2D>`_
    to determine an adaptive background that takes into account variations in flux
    across the image.
...
def classify_sources(catalog, fwhm, sources=None):
    ...
    Parameters
    ----------
    catalog : `photutils.segmentation.SourceCatalog <https://photutils.readthedocs.io/en/stable/api/photutils.segmentation.SourceCatalog.html#photutils.segmentation.SourceCatalog>`_
...
def generate_source_catalog(image, dqname="DQ", output=False, fwhm=3.0,
    ...
    Parameters
    ----------
    image : `astropy.io.fits.HDUList <https://docs.astropy.org/en/stable/io/fits/api/hdulists.html>`_
...
def generate_sky_catalog(image, refwcs, dqname="DQ", output=False):
    ...
    image : `astropy.io.fits.HDUList <https://docs.astropy.org/en/stable/io/fits/api/hdulists.html>`_
        Input image.
    refwcs : `stwcs.wcsutil.HSTWCS <https://stwcs.readthedocs.io/en/latest/hstwcs.html>`_
...
def compute_photometry(catalog, photvals):
    ...
    Parameters
    ----------
    catalog : `astropy.table.Table <https://docs.astropy.org/en/stable/api/astropy.table.Table.html>`_
    ...
    Returns
    -------
    phot_cat : `astropy.table.Table <https://docs.astropy.org/en/stable/api/astropy.table.Table.html>`_
...
def filter_catalog(catalog, bright_limit=1.0, max_bright=None, min_bright=20, colname="vegamag"):
    ...
    Returns
    -------
    new_catalog : `astropy.table.Table <https://docs.astropy.org/en/stable/api/astropy.table.Table.html>`_
...
def build_self_reference(filename, clean_wcs=False):
    ...
    clean_wcs : bool
        ...
        the output from ``utils.output_wcs()`` into a pristine `stwcs.wcsutil.HSTWCS <https://stwcs.readthedocs.io/en/latest/hstwcs.html>`_ object.

    Returns
    -------
    customwcs : `stwcs.wcsutil.HSTWCS <https://stwcs.readthedocs.io/en/latest/hstwcs.html>`_
...
def read_hlet_wcs(filename, ext):
    """Insure `stwcs.wcsutil.HSTWCS <https://stwcs.readthedocs.io/en/latest/hstwcs.html>`_ includes all attributes of a full image WCS.
...
def build_hstwcs(crval, crpix, naxis1, naxis2, pscale, orientat):
    """ Create an `stwcs.wcsutil.HSTWCS <https://stwcs.readthedocs.io/en/latest/hstwcs.html>`_ object for a default instrument without
...
def within_footprint(img, wcsobj, x, y):
    ...
    wcsobj : `stwcs.wcsutil.HSTWCS <https://stwcs.readthedocs.io/en/latest/hstwcs.html>`_
...
def find_hist2d_offset(filename, reference, refwcs=None, refnames=['ra', 'dec'],
    ...
    Parameters
    ----------
    filename : `astropy.io.fits.HDUList <https://docs.astropy.org/en/stable/io/fits/api/hdulists.html>`_ or str
        Single image to extract sources for matching to
        the external astrometric catalog.

    reference : str or `astropy.table.Table <https://docs.astropy.org/en/stable/api/astropy.table.Table.html>`_
    ...
    -------
        ...
        seg_xy, ref_xy : `astropy.table.Table <https://docs.astropy.org/en/stable/api/astropy.table.Table.html>`_
...
def build_wcscat(image, group_id, source_catalog):
    ...
    objects for all chips in an image.

    Parameters
    ----------
    image : str, `astropy.io.fits.HDUList <https://docs.astropy.org/en/stable/io/fits/api/hdulists.html>`_
    ...
    source_catalog : dict
        ...
        entries in each chip's ``FITSWCSCorrector`` object.  It should be provided as a
        dict of astropy Tables identified by chip number with
    ...
    Returns
    -------
    wcs_catalogs : list of `tweakwcs.correctors.FITSWCSCorrector <https://tweakwcs.readthedocs.io/en/latest/source/correctors.html#tweakwcs.correctors.FITSWCSCorrector>`_
        List of
        `tweakwcs.correctors.FITSWCSCorrector <https://tweakwcs.readthedocs.io/en/latest/source/correctors.html#tweakwcs.correctors.FITSWCSCorrector>`_
...
def get_epoch(hdulist):
    ...
    Parameters
    ----------
    hdulist : `astropy.io.fits.HDUList <https://docs.astropy.org/en/stable/io/fits/api/hdulists.html>`_
